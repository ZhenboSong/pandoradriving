...
def placeholder_inputs_img5(batch_size, img_rows=66, img_cols=200, separately=False):
    imgs_pl = tf.placeholder(tf.float32, shape=(batch_size, img_rows, img_cols, 5))
    if separately:
        speeds_pl = tf.placeholder(tf.float32, shape=(batch_size))
        angles_pl = tf.placeholder(tf.float32, shape=(batch_size))
        labels_pl = [speeds_pl, angles_pl]
...
def get_loss_mae(pred, label, l2_weight=0.0001):
    diff = tf.abs(tf.subtract(pred, label))
...
def get_loss_mse(pred, label, l2_weight=0.0001):
...
def get_loss_stlossmse(pred, label, l2_weight=0.0001):
    diff = tf.square(tf.subtract(pred, label))
    loss = (1+tf.abs(label)/180.0)*diff
...
def get_loss_stloss2(pred, label, l2_weight=0.0001):
    diff = tf.abs(tf.subtract(pred, label))
    
    beta = 1.0
    
    quadratic = tf.minimum(diff, beta)
    linear = (diff - quadratic)
    loss = 0.5 * quadratic**2 + beta * linear
    
    loss = (1+tf.abs(label)/180.0)*loss
